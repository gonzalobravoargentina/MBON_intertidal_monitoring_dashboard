...
# Leer archivos de cobertura
cover_arg <- read_csv(file.path(Data, "percent_covers.csv"))
cover_chile <- read_csv(file.path(Data, "percent_covers_chile.csv"))

# Leer archivos de metadata
metadata_arg <- read_csv(file.path(Data, "metadata.csv"))
metadata_chile <- read_csv(file.path(Data, "metadata_chile.csv"))

# Leer AMP_list
AMP_list <- read_csv(file.path(Data, "AMP_list.csv"))

# Unir los datasets de cobertura y metadata de Argentina y Chile
cover_all <- bind_rows(cover_arg, cover_chile)
metadata_all <- bind_rows(metadata_arg, metadata_chile)

# Hacer el merge de metadata y cover
AMP <- merge(metadata_all, cover_all, by.x = "Name", by.y = "Image name", all.x = TRUE)

# Remover data frames originales para liberar memoria
rm(cover_arg, cover_chile, metadata_arg, metadata_chile, cover_all, metadata_all)
...
for (localidad in localidades_unicas) {
  ...
  p <- plot_ly(labels = ~CATAMI, values = ~cover.mean, legendgroup = ~CATAMI, textinfo = 'label+percent',
    ...
    layout(title = localidad, showlegend = TRUE,
           grid = list(rows = 1, columns = 3),
           xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
           yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
  
  # Agregar el gráfico a la lista usando el nombre de la localidad como clave
  plot_list[[localidad]] <- p
}
...
```{r}
plot_list[["VALPARAISO"]]
```

### PUNTA DEL DIABLO

```{r}
plot_list[["PUNTA DEL DIABLO"]]
```
...
```{r}
plot_list[["CONCEPCION"]]
```

### MAR DEL PLATA

```{r}
plot_list[["MAR DEL PLATA"]]
```
...
```{r}
plot_list[["ISLOTE LOBOS"]]
```

### PUNTA BS AS

```{r}
plot_list[["PUNTA BS AS"]]
```

### PUNTA LOMA

```{r}
plot_list[["PUNTA LOMA"]]
```

### PUNTA TOMBO

```{r}
plot_list[["PUNTA TOMBO"]]
```

### CABO DOS BAHIAS

```{r}
plot_list[["CABO DOS BAHIAS"]]
```
...
```{r}
plot_list[["PIMCPA_ISLA TOVA"]]
```

### PUNTA BUQUE

```{r}
plot_list[["PUNTA BUQUE"]]
```
...
```{r}
plot_list[["MONTE LEON"]]
```

### RESERVA COSTA ATLANTICA

```{r}
plot_list[["RESERVA COSTA ATLANTICA"]]
```
...
```{r Heatmaps_plot_code, message=FALSE, warning=FALSE, include=FALSE}
library(ggplot2)
library(dplyr)
library(tidyr)
library(forcats)
...
# Función para procesar datos y generar gráficos por país
procesar_y_graficar <- function(data, country_name) {
  
  # Filtrar solo las localidades del país seleccionado
  data_filtered <- data %>%
  ...
  # Crear estructura completa de localidades y estratos para asegurar que todas aparezcan
  localidades_completas <- expand_grid(
    locality = unique(data_filtered$locality),
    strata = factor(c("LOWTIDE", "MIDTIDE", "HIGHTIDE"), levels = c("LOWTIDE", "MIDTIDE", "HIGHTIDE")),
    Categoria = unique(heatmap_data$Categoria)
  )
  
  # Unir con la estructura original para completar los datos faltantes
  heatmap_data <- localidades_completas %>%
    left_join(heatmap_data, by = c("locality", "strata", "Categoria"))
  
  # Ordenar las localidades según la latitud promedio original
  orden_localidades <- heatmap_data %>%
    group_by(locality) %>%
    summarise(lat_media = mean(Latitude, na.rm = TRUE), .groups = "drop") %>%
    arrange(desc(lat_media)) %>%
    pull(locality)
  ...
  # Graficar heatmaps asegurando ejes iguales en todos los paneles
  p <- ggplot(heatmap_data, aes(x = locality, y = strata, fill = Cobertura)) +
    geom_tile() +
    scale_fill_viridis_c(option = "plasma", na.value = "white") + # NA en blanco
    labs(title = paste("Cobertura (%) por Localidad y Estrato en", country_name),
         x = "Localidad", y = "Estrato", fill = "Cobertura (%)") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          panel.spacing = unit(1, "lines")) +
  ...
  # Convertir el gráfico a interactivo y devolverlo
  return(ggplotly(p))
}
```

### ARGENTINA
```{r Heatmaps_arg, message=FALSE, warning=FALSE}
p_arg <- procesar_y_graficar(AMP, "ARGENTINA")

p_arg
```

### CHILE   
```{r Heatmaps_chile, message=FALSE, warning=FALSE}
p_chile <- procesar_y_graficar(AMP, "CHILE")
...
### URUGUAY
```{r Heatmaps_ury, message=FALSE, warning=FALSE}
p_uruguay <- procesar_y_graficar(AMP, "URUGUAY")
...
#AMP_heatmap <- subset(AMP_heatmap,locality!="VALPARAISO")
# Diccionario de nombres completos
catami_nombres <- c(
